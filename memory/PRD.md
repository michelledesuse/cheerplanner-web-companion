...
- STATUS: All code + non-generation flows verified (endpoints, UI screenshots, delete). LIVE Imagen 4 generation BLOCKED: user's Gemini key project shows zero imagen/predict models (billing not yet active). App returns the correct guidance message; will work once billing propagates. Key confirmed by user (value stored only in backend/.env as GEMINI_API_KEY; not recorded here).

## Session update 22b (Imagen 4 flyer — final state: Imagen-only per user)
- Per user, reverted the auto-fallback: flyer generator is **Imagen 4 ONLY** (no Gemini-image fallback). routers/coach_ai.py generate_flyer uses GeminiImageGeneration(model=IMAGEN_MODEL default imagen-4.0-fast-generate-001); on model-not-found returns 503 "requires paid-tier project; link billing to the exact project the key belongs to". _gemini_flash_image helper removed.
- ROOT CAUSE of Imagen 404 identified: user's key `AQ.Ab8...` (53 chars) is NOT a standard Gemini API key (those are `AIzaSy...`, ~39 chars). It's an ephemeral/OAuth-style token — authenticates + lists 50 generateContent models + gemini-2.5-flash-image works, but has NO imagen/predict access even though AI Studio plan shows "Paid". ListModels (v1 & v1beta) return zero imagen/predict models; direct :predict REST returns 404 NOT_FOUND.
- ACTION NEEDED FROM USER (still pending): create a standard API key at aistudio.google.com/apikey in the PAID project (must start with `AIzaSy`) and provide it. Feature will then work with no code changes.
- Everything else VERIFIED working: Team name field, "Let AI design the layout" toggle, logo picker (upload OR choose saved via db.flyer_logos + GET /logos), Media Library (renamed) with per-item + preview delete (DELETE /team/coach-ai/flyers/{id} → removes flyers+chat_media+storage via core.storage.delete_object), post-to-chat, staff-only chat deletion. Live image gen confirmed possible on the key via gemini-2.5-flash-image (762KB img) proving key validity; test flyer cleaned from demo.

## Session update 22c (Flyer generator FINAL — Imagen 4 primary + auto-fallback, verified iter114)
- User confirmed (option a): flyer generator now uses **Imagen 4 as primary with automatic server-side fallback to gemini-2.5-flash-image**. routers/coach_ai.py generate_flyer: tries GeminiImageGeneration(model=IMAGEN_MODEL); on model-not-found (404/not supported) calls _gemini_flash_image(api_key, prompt) which uses google.genai generate_content on GEMINI_IMAGE_MODEL (default gemini-2.5-flash-image) and extracts inline_data bytes. Imagen 4 auto-takes-over if it ever becomes available to the account — no code change needed. Auth (503)/quota (429) errors still surfaced distinctly.
- WHY fallback: Imagen 4 is NOT reachable for this user's Google account — verified exhaustively: (1) Emergent Universal Key invalid for Gemini image playbook (playbook + live 'API key not valid'); (2) two user Google keys (AQ.* format, Paid 1 tier, project 'CheerPlanner App Chat') both list 50 models but ZERO imagen/predict → region/account restriction; (3) Vertex AI path abandoned (user couldn't complete service-account setup). gemini-2.5-flash-image works on the user's key (live ~2.5MB image).
- VERIFIED iter114: 14/14 backend (generate ~7-10s real image, staff-only 403 gating on flyer/logos/flyers/delete/chat-delete, Media Library CRUD, post-to-chat, delete→404) + full frontend (all testIDs, generate→image, Media Library per-item delete, logo picker upload/choose-saved). Demo 'Glitter Gala Fundraiser' preserved; test flyers cleaned.
- Config: backend/.env GEMINI_API_KEY=YOUR_GCP_API_KEY, IMAGEN_MODEL=imagen-4.0-fast-generate-001, GEMINI_IMAGE_MODEL=gemini-2.5-flash-image (fallback).

## Session update 23 (CheerPlanner AI Designer — OpenAI GPT-Image-2, staff-only, verified iter115)
- NEW feature "AI Designer" (Team Hub tool, key ai_designer, route /team/ai-designer, icon color-palette-outline). Staff-only: Team Hub is team_access-gated + backend require_team_access + client-side redirect guard (non-staff → /team).
- Backend routers/ai_designer.py: uses the USER's own OpenAI key (OPENAI_API_KEY in backend/.env) calling the official openai SDK directly (client.images.generate model=gpt-image-2, size default 1024x1024, b64_json). NOT the emergent wrapper (which only supports gpt-image-1/dall-e-3). AI_DESIGNER_MODEL env override. Endpoints: POST /api/ai-designer/generate {prompt,size?}→{image_base64,prompt,size}; POST /save {image_base64,prompt}→{design_id} (Object Storage + db.ai_designs w/ JPEG thumb); GET /designs; GET /designs/{id}→full image_base64; DELETE /designs/{id} (removes doc+storage). Clear error mapping: org-not-verified/model-access→503, content policy→400, quota/billing→402, rate limit→429, bad key→503.
- Frontend app/team/ai-designer.tsx: 'What do you want to create?' + large prompt box + Generate Design + loading/progress + generated image + Regenerate/Save/Share. Share via expo-sharing (native share sheet) / web download of data URL. 'My Designs' library sheet (header images icon) lists saved designs w/ thumbs, open full-size, delete. api calls use timeout:120000 (generate) / 60000 (save/get).
- Future-ready (not built yet per spec): reference image upload, logo upload, edit existing image, transparent bg, multiple variations.
- VERIFIED iter115: backend 9/9 pytest (real gpt-image-2 generate ~13-17s, save/list/get/delete, non-staff 403, unauth 401, empty-prompt 400) + full frontend E2E (generate→loading→image→Save→Saved→Share download→library open/delete, Regenerate present). No monetization. Existing features untouched.
- Config: backend/.env OPENAI_API_KEY=YOUR_OPENAI_API_KEY (user's own key; value stored only in .env). GPT-Image-2 confirmed working live on the user's key (org verification was 'in process' but generation succeeds).

## Session update 24 (AI Designer enhancements — reference/logo, variations, edit, transparent — verified iter116)
- Reference & Logo upload: frontend BrandSlot slots (ai-designer-ref-add / ai-designer-logo-add, thumbnails + clear) pick via expo-image-picker (base64). When any image input present, backend routes to OpenAI images.edit (gpt-image-2) instead of images.generate.
- Design Variations: chips 1-4 (ai-designer-var-N). Backend generates N images IN PARALLEL (asyncio.gather over run_in_threadpool → wall-time bounded ~25-35s regardless of N; verified var=4 via external proxy 200 in ~26s). Response now {images:[...], image_base64, prompt, size}. Frontend shows a selectable variation strip (ai-designer-variation-i); Save/Share/Regenerate act on selected.
- Edit a Design: 'Tweak this design' input (ai-designer-tweak) + apply (ai-designer-apply-edit) resends the CURRENT selected image as edit_image with the follow-up prompt via images.edit.
- Transparent backgrounds: toggle (ai-designer-transparent) → passes background='transparent' to gpt-image-2 (PNG, no bg — for logos/stickers).
- Backend routers/ai_designer.py generate now accepts {prompt, variations(1-4, MAX_VARIATIONS=4), transparent, reference_images[], logo, edit_image}; _decode_inputs merges edit_image+reference_images+logo (MAX_INPUT_IMAGES=4, handles data: prefixes); prompt optional when images present (default redesign prompt). Same error mapping (verify/policy/quota/rate/auth). Frontend api timeout raised to 180000 for generate.
- VERIFIED iter116: backend 14/14 pytest (9 legacy + 5 new: var2+transparent=2 PNGs, clamp to 4, edit_image, ref+logo data-URL, ref-only-no-prompt) + full frontend E2E (all new controls, var strip selection, transparent, tweak→new image, save/library/delete, non-staff 403). Real gpt-image-2 throughout. No monetization.

## Session update 25 ("Design a Flyer" — rename + chat post + enlarge + brand presets + download + premium — verified iter117)
- RENAMED "AI Designer" → "Design a Flyer" (Team Hub tool title + screen header). Route unchanged (/team/ai-designer).
- MONETIZE: tool added to frontend PREMIUM_TOOLS (shows PREMIUM badge + routes to /premium when gatingActive). Backend endpoints (generate/save/post-to-chat/brands create/update) call core.gating.assert_premium(user, "ai_designer") — no-op pre-launch (monetization_active false), enforces 402 premium_required once monetization is on.
- POST TO CHAT: one-tap `POST /api/ai-designer/post-to-chat` {image_base64, caption?} uploads chat_media + inserts team_messages (image). Frontend button under result + in enlarge modal (ai-designer-post-chat / -enlarge-post). Deletion uses EXISTING team_chat rule: coach/staff (team_access) can delete any hub message incl. coach-posted flyers; parents/athletes can only delete their own (403 otherwise) — verified.
- ENLARGE: tap image → full-screen modal (ai-designer-enlarge-modal) w/ Download, Share, Post, Close.
- BRAND PRESETS: db.brand_presets, endpoints GET/POST/PATCH/DELETE /api/ai-designer/brands ({name, colors[], logo downscaled 512px}). Multiple brands supported; create/edit(long-press)/delete via modal (ai-designer-brand-*). Selecting a chip sends brand_id to generate → backend applies logo (if none) + appends "team <name> and brand colors <...>" to the prompt.
- DOWNLOAD: explicit button (ai-designer-download) — web anchor download; native saves to Photos via expo-media-library (added; app.json: NSPhotoLibraryAddUsageDescription + Android WRITE_EXTERNAL_STORAGE). Share retained inside enlarge modal.
- VERIFIED iter117: backend 26/26 pytest (brand CRUD, post-to-chat, non-staff 403 on all new endpoints, parent cannot delete coach msg, all iter115/116 regressions) + full frontend E2E (rename, brand CRUD, generate→enlarge→download/share/post, post-to-chat shows in Team Chat, coach delete). Real gpt-image-2 throughout. Test data cleaned. Known: PREMIUM badge only visible once monetization_active flips on (by design).

## Session update 26 (Apple-review demo seed for new features)
- Extended scripts/seed_marketing_demo.py (demo@cheerplanner.app) to also seed the new features, idempotently (added household-scoped wipes for team_events, calendar_rsvps, ai_designs, brand_presets, chat_media).
- Seeded: 3 Team Hub calendar events (practice/competition/fundraiser, upcoming); a duplicated Team Form "Banquet Meal Order (copy)"; Design-a-Flyer content = 1 brand preset "Champion Elite Allstars" (colors navy/gold + generated transparent logo) + 3 real GPT-Image-2 designs in My Designs (Regional Competition Poster, Tryouts 2026 Flyer, Season Banquet Invite) + 1 flyer posted into Team Chat.
