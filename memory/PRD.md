...
- S1: **Precise SMS lead-time reminders for time-sensitive events.** SMS-only (NOT email). User can multi-select any/all of these lead times: **1 hr before, 30 min before, 15 min before, 1 min before.** Applies to two triggers:
    - **Stay-to-play booking opening** — remind before the booking window opens (uses the event's booking-open datetime).
    - **Flight check-in** — check-in opens **24 hours before the flight**; reminders fire relative to that check-in-open time (i.e. 1hr/30/15/1 min before the 24h-before mark).
  IMPLEMENTATION NOTES: current SMS pipeline is an hourly digest (core/scheduler.py :05) — minute-level precision (30/15/1 min) requires a finer-grained scheduler tick (e.g. every 1 min) or per-reminder scheduled jobs, plus storing the exact target datetime + selected offsets per booking/flight and a sent-dedupe per offset. Store opt-in offsets on the relevant record (stay-to-play booking, flight) and/or notification prefs. Respect existing SMS opt-in/consent + STOP. Confirm timezone handling (event local time vs user tz).
