...
3. **Expenses** – per athlete, categorized. Each expense exposes `paid_amount` (sum of linked payments) and `balance_due`. Supports **partial payments**: tap the per-row "Apply" button to open a bottom-sheet and apply funds from manual cash or any fundraiser with available balance. Fully covered expenses auto-flip to `paid=true`.
4. **Payments** – per athlete with method tracking (Card, Bank, Cash, Fundraiser, Other). Linkable to one or more expenses (`applied_expense_ids`). Mark-paid auto-flip now only fires when cumulative payments fully cover the expense.
