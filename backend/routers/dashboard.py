...
from core.helpers import _build_paid_map, parse_date, _member_visibility, _household_user_ids
...
async def dashboard(current_user=Depends(get_current_user)):
    ...
    athletes_count = await db.athletes.count_documents({"user_id": hh})
    comps_count = await db.competitions.count_documents({"user_id": hh})

    # Total expenses & payments YTD
    total_expenses = 0.0
    async for d in db.expenses.find({"user_id": hh}, {"_id": 0, "amount": 1}).limit(20000):
    ...
    # Booking balances
    booking_balance = 0.0
    async for d in db.bookings.find({"user_id": hh}, {"_id": 0, "cost": 1, "amount_paid": 1, "balance_due_date": 1}).limit(5000):
    ...
    async for d in db.expenses.find(
        {"user_id": hh}, {"_id": 0, "id": 1, "amount": 1, "paid": 1, "due_date": 1}
    ).limit(20000):
    ...
    # Next competition
    next_comp = None
    async for d in db.competitions.find({"user_id": hh}, {"_id": 0}).sort("event_date", 1):
    ...
    # Fundraisers total
    total_raised = 0.0
    async for d in db.fundraisers.find({"user_id": hh}, {"_id": 0, "amount_raised": 1}).limit(5000):
    ...
    month_spend = 0.0
    async for d in db.expenses.find(
        {"user_id": hh, "incurred_on": {"$regex": f"^{this_month}"}},
        {"_id": 0, "amount": 1},
    ).limit(20000):
    ...
    seasons_count = await db.seasons.count_documents({"user_id": hh})
    if seasons_count == 0:
        dates = [c.get("event_date") async for c in db.competitions.find(
            {"user_id": hh}, {"_id": 0, "event_date": 1}).limit(20000)]
