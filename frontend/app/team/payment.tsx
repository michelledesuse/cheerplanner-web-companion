...
type Entry = { member_id: string; paid: boolean; amount_paid?: number | null };
...
export default function PaymentDetail() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setTracker(t.data); setRoster(r.data);
  ...
  const toggle = async (mid: string) => {
    if (!tracker) return;
    const cur = entryFor(mid);
    setBusy(mid);
    try {
      const r = await api.put<Tracker>(`/team/payments/${tracker.id}/member/${mid}`, { paid: !cur?.paid });
      setTracker(r.data);
    } catch (e: any) {
  ...
  const remove = () => {
    if (!tracker) return;
    Alert.alert("Delete tracker?", "This removes it and all paid marks.", [
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 100 }} testID="payment-detail">
        <View style={styles.summaryCard}>
          {tracker.amount != null && <Text style={styles.summaryAmount}>{formatCurrency(tracker.amount)} per person</Text>}
        ...
        {roster.length === 0 ? (
          <View style={styles.emptyBlock}>
            <Text style={styles.emptyText}>Add people to your Roster first &mdash; they&apos;ll appear here to check off.</Text>
          </View>
        ) : roster.map((m) => {
          const e = entryFor(m.id);
          const paid = !!e?.paid;
          return (
            <TouchableOpacity key={m.id} style={styles.memberRow} onPress={() => toggle(m.id)} disabled={busy === m.id} testID={`payment-member-${m.id}`}>
              <View style={[styles.check, paid && styles.checkOn]}>
                {busy === m.id ? <ActivityIndicator size="small" color={paid ? "white" : colors.accent} /> : paid ? <Ionicons name="checkmark" size={16} color="white" /> : null}
              </View>
              <Text style={[styles.memberName, paid && styles.memberNamePaid]}>{m.name}</Text>
              <Text style={[styles.statusText, { color: paid ? colors.successText : colors.textTertiary }]}>{paid ? "Paid" : "Unpaid"}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
...
const makeStyles = (c: ThemePalette) => ({
  ...
  memberName: { ...typography.bodyMedium, color: c.textPrimary, flex: 1 },
  memberNamePaid: { color: c.textSecondary },
